...
Deno.test("ECS Auto-Scaling Configuration Files", async (test) => {
  await test.step("CloudFormation template exists", () => {
    const filePath = `${projectRoot}/ecs-autoscaling.cloudformation.yaml`;
    assertEquals(
      existsSync(filePath),
      true,
  ...
  await test.step("Terraform configuration exists", () => {
    const filePath = `${projectRoot}/ecs-autoscaling.tf`;
    assertEquals(
      existsSync(filePath),
      true,
  ...
  await test.step("Example config JSON exists and is valid", () => {
    const filePath = `${projectRoot}/ecs-autoscaling-config.example.json`;
    assertEquals(
      existsSync(filePath),
      true,
  ...
  await test.step("Documentation exists", () => {
    const filePath = `${projectRoot}/docs/ECS-AUTOSCALING.md`;
    assertEquals(
      existsSync(filePath),
      true,
  ...
  await test.step("Deployment script exists and is executable", () => {
    const filePath = `${projectRoot}/dev/deploy-autoscaling.sh`;
    assertEquals(
      existsSync(filePath),
      true,
  ...
  await test.step("README exists", () => {
    ...
    assertEquals(
      existsSync(filePath),
      true,
      "Auto-scaling README should exist"
    );
  });
